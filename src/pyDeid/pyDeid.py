...
def pyDeid(
    ...
    **custom_regexes
    ...
    if (
        custom_dr_first_names
        or custom_dr_last_names
        or custom_patient_first_names
        or custom_patient_last_names
    ):
    ...
    if custom_regexes:
        for custom_regex_id in custom_regexes:
            ...
            builder.set_custom_regex(
                custom_reg.pattern,
                custom_reg.phi_type,
                custom_reg.surrogate_builder_fn,
                evaluated_args,
            )
...
def deid_string(
    ...
    **custom_regexes
    ...
    builder = pyDeidBuilder().replace_phi().set_phi_types(types)
...
def display_deid(original_string, phi):
    ...
    for ent in phi:
        ...
        if any(re.search("MRN|SIN|OHIP", line) for line in ent["types"]):
            ...
            re.search("Address|Location|Postalcode", line) for line in ent["types"]
        ):
            e["label"] = "LOC"

        elif any(
            re.search("(First Name)|(Last Name)|(Name Prefix)|(Name)", line)
            for line in ent["types"]
        ):
            e["label"] = "NAME"

        elif any(
            re.search(r"day|month|year|Holiday", line, re.IGNORECASE)
            for line in ent["types"]
        ):
            e["label"] = "DATE"
