...
class PHIPruner:
    ...
    def prune_phi(self) -> Dict[PHI, List[str]]:
        ...
        phi_keys = sorted(
            self.phis.keys(), key=lambda x: x.start
    ...
    def _check_ambiguous_types(self, phi_keys: List[PHI]) -> None:
        ...
        for i in range(len(phi_keys)):
            ...
            if i != 0:
                ...
                if (
                    (
                        is_type(prev_key, "Name", 1, self.phis)
                        and is_type(current_key, "Name", 1, self.phis)
                    )
                    # and (not is_common(current_key.phi) or not is_common(prev_key.phi))
                    and not is_common(current_key.phi)
                    and not is_common(prev_key.phi)
                    and not re.search(r"\.", prev_key.phi)
                    and not ((current_key.end - prev_key.start) < 3)
                ):
                    if (
                        (
                            is_ambig(current_key, self.phis)
                            and is_ambig(prev_key, self.phis)
                            and is_type(prev_key, "First Name", 1, self.phis)
                            and is_type(current_key, "Last Name", 1, self.phis)
                        )
                        or (
                            not is_ambig(current_key, self.phis)
                            and is_ambig(prev_key, self.phis)
                            and is_type(prev_key, "First Name", 1, self.phis)
                            and is_type(current_key, "Last Name", 1, self.phis)
                        )
                        or (
                            is_ambig(current_key, self.phis)
                            and not is_ambig(prev_key, self.phis)
                            and is_type(prev_key, "First Name", 1, self.phis)
                            and is_type(current_key, "Last Name", 1, self.phis)
                        )
                    ):
                    ...
                    if (
                        not is_ambig(current_key, self.phis)
                        and is_ambig(prev_key, self.phis)
                        and is_type(current_key, "First Name", 1, self.phis)
                        and is_type(prev_key, "Last Name", 1, self.phis)
                    ) or (
                        is_ambig(current_key, self.phis)
                        and not is_ambig(prev_key, self.phis)
                        and is_type(current_key, "First Name", 1, self.phis)
    ...
    def _remove_overlapping_phi(self, phi_keys: List[PHI]) -> None:
        ...
        for i in range(len(phi_keys)):
            ...
            if i != 0:
                ...
                self._check_overlap_and_add_bad_keys(
                    previous_key, current_key, bad_keys
                )
    ...
    def _check_overlap_and_add_bad_keys(
        self, previous_key: PHI, current_key: PHI, bad_keys: List[PHI]
    ) -> None:
        """
        ...
        # if found PHI is part of a medical phrase, ignore it to reduce false positives
            ...
            re.search("MedicalPhrase", val) for val in self.phis[current_key]
        ) and (current_start <= previous_start and current_end >= previous_end):
            bad_keys.extend([previous_key, current_key])

        elif any(
            re.search("MedicalPhrase", val) for val in self.phis[previous_key]
        ) and (
            current_start > previous_start
            and current_start < previous_end
            and current_end > previous_end
    ...
    def _should_remove_previous_key(self, previous_key: PHI, current_key: PHI) -> bool:
        ...
        """
        return (
            any(re.match(r"^Day Month \[", x) for x in self.phis[previous_key])
            and any(re.match(r"^Day Month Year \[", x) for x in self.phis[current_key])
            or any(re.match(r"^Month Day \[", x) for x in self.phis[previous_key])
            and any(re.match(r"^Month Day Year \[", x) for x in self.phis[current_key])
        )

    def _combine_and_replace_keys(
        self, previous_key: PHI, current_key: PHI, bad_keys: List[PHI]
    ) -> None:
        """
        ...
        new_key = PHI(
            previous_key.start,
            current_key.end,
            self.note[previous_key.start : current_key.end],
        )
    ...
    def _combine_overlapping_dates(self, phi_keys: List[PHI]):
        ...
        for i in range(len(phi_keys)):
            for j in range(i + 1, len(phi_keys)):
                ...
                if any(
                    re.search(r"(Date|Year|Month|Day|Time)", v, re.IGNORECASE)
                    for v in vals1
                ) and any(
                    re.search(r"(Date|Year|Month|Day|Time)", v, re.IGNORECASE)
                    for v in vals2
                    ...
                    if not (key1.end <= key2.start or key2.end <= key1.start):
                        ...
                        return self._combine_overlapping_dates(
                            sorted(self.phis.keys(), key=lambda x: x.start)
                        )
