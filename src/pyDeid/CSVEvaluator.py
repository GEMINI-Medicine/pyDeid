...
class CSVEvaluator:
    ...
    def add_ground_truth_file(
        ...
        if ignore_annotations:
            ground_truth["annotation"] = ground_truth["annotation"].replace(
                ignore_annotations, np.nan
            )
    ...
    def validate_and_read_file(filepath, columns) -> pd.DataFrame:
        ...
        missing_cols = [
            col for col, name in columns.items() if name and name not in df.columns
        ]
    ...
    def combine_files(self) -> None:
        ...
        merge_misses = merge_misses[
            (
                (
                    (
                        merge_misses[res_cols["phi_start"]]
                        <= merge_misses[gt_cols["start"]]
                    )
                ...
                | (
                    ...
                    & (
                        merge_misses[res_cols["phi_end"]]
                        >= merge_misses[gt_cols["end"]]
                    )
                )
            )
        ...
        res = to_analyze[
            ~to_analyze.set_index(["genc_id", "start", "end"]).index.isin(
                merge_misses.set_index(["genc_id", "start", "end"]).index
            )
        ]

        res = res[
            ~res.set_index(["encounter_id", "phi_start", "phi_end"]).index.isin(
                merge_misses.set_index(["encounter_id", "phi_start", "phi_end"]).index
            )
        ]
    ...
    def standardize_phi(self) -> None:
        ...
        def categorize_deid(annotation: Any) -> Optional[str]:
            ...
            if any(
                ...
                term in str(annotation)
                for term in ["MRN", "Medical Record Number", "SIN", "OHIP"]
            ):
                return "ID"
            elif any(
                term in str(annotation)
                for term in ["Telephone", "Email", "E-mail", "Fax"]
            ):
                return "Contact"
            elif any(
                term in str(annotation) for term in ["Location", "Street", "Postal"]
            ):
                return "Location"
            return None
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        true_positives = df[
            (df.manual_type.notnull())
            & (df.deid_type.notnull())
            & (df.manual_type == df.deid_type)
        ].shape[0]
        false_positives = df[
            (df.manual_type.isnull()) & (df.deid_type.notnull())
        ].shape[0]
        false_negatives = df[
            (df.manual_type.notnull()) & (df.deid_type.isnull())
        ].shape[0]
...
def melt_annotations(
    input_file: str, output_file: str, merge_annotations: Optional[List[str]] = None
) -> None:
    """
    ...
    with open(input_file, "r", newline="") as infile, open(
        ...
        for row in reader:
            ...
            should_merge = (merge_annotations is None and prev_row["annotation"]) or (
                merge_annotations is not None
                and prev_row["annotation"] in merge_annotations
            )

            if (
                should_merge
                and row["annotation"] == prev_row["annotation"]
                and int(row["start"]) - int(prev_row["end"]) < 3
            ):
                # Merge tokens
